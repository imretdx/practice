...
public class AttributeReflectionTests
{
    ...
    [Fact]
    public void PrintTypeInfo_ReturnsCorrectFormattedString()
    {
        var result = ReflectionHelper.PrintTypeInfo(typeof(SampleClass));
        Assert.Contains("Класс: Пример класса", result);
        Assert.Contains("Версия: 1.0", result);
        Assert.Contains("Свойство: Числовое свойство", result);
        Assert.Contains("Метод: Тестовый метод", result);
    }
}
